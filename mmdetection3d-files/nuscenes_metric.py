...
class NuScenesMetric(BaseMetric):
    ...
    def _evaluate_single(
        ...
        nusc_eval = NuScenesEval(
            ...
            verbose=False)
        nusc_eval.main(plot_examples=50, render_curves=False)   # Changed for UNION.
    ...
    def format_results(
        ...
        import numpy as np   # Added for UNION.
        results_dir = osp.join(jsonfile_prefix, 'raw_results_nusc.npy')   # Added for UNION.
        np.save(results_dir, results)   # Added for UNION.
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = self.DefaultAttribute[name] if name in list(self.DefaultAttribute.keys()) else 'none'   # Changed for UNION.
                else:
...
def lidar_nusc_box_to_global(
    ...
    for box in boxes:
        ...
        det_range = cls_range_map[classes[box.label]] if classes[box.label] in list(cls_range_map.keys()) else 50   # Changed for UNION.
        if radius > det_range:
            continue
